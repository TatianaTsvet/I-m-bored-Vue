...
<NewActivityModal @openModal='openModal' v-model:show='modalVisible' v-model:randomActivity='randomActivity' @addToFavorites='addToFavorites'/>
<!-- <Spinner/> -->
</div>
</template>
...
export default defineComponent({  
  name: 'App',
  components: {
    Header, Cards, NewActivityModal, Spinner,
  },
  data() {
    return {
      modalVisible: false,
  ...
  findActivity (stateActivity: IActivity[], activityKey: number):void {
    ...
    getActivity(newActivity:IActivity) {
      ...
      const sameHistoryActivity = !!this.findActivity(this.historyActivity, newActivity.key);
      this.historyActivity = sameHistoryActivity ? this.historyActivity : [...this.historyActivity, randomActivity];
    ...
    const sameActivity = !!this.likedActivity.find((item: {key:number})=> item.key === favoriteActivity.key);
     this.likedActivity = sameActivity ? this.likedActivity : [...this.likedActivity, likeActivity];
